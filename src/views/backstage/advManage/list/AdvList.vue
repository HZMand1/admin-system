<template>
  <section>
    <section v-loading="loading" element-loading-text="拼命加载中……" class="bg-white pad-t-30 pad-r-10 pad-b-30 pad-l-10">
      <div class="table-filter-wrapper mar-l-10">
        <ph-form @searchBtn="searchBtn" @resetSearchBtn="resetSearchBtn" @addHandle="addHandle" :searchForm="searchForm">
        </ph-form>
      </div>
      <div class="table-wrapper pad-t-5">
        <ph-table :tableData="tableData" @editHandle="editHandle" @singleDeleteHandle="singleDeleteHandle" @selectionChangeHandle="selectionChangeHandle">
        </ph-table>
        <el-row class="mar-t-10">
          ...
          <el-col :span="24">
            <ph-pagination @sendPaginations="sendPaginationsHandle" :paginations="paginationParams" class="mar-r-20 text-right">
            </ph-pagination>
          </el-col>
        </el-row>
      </div>
    ...
    <!--新增、编辑-->
    <added-editor-dialog :dialogForm="dialogForm" :submitLoading="submitLoading" :dialogVisible.sync="addedEditorDialog" @getCancel="handleCancel" @closeBindWarnStandard="handleCancel" @submitHandle="submitAddedEditorForm">
    </added-editor-dialog>
  </section>
</template>
...
export default {
  ...
  methods: {
    ...
    searchBtn(searchData) {
      ...
      let addTime =
        searchData && searchData.time[0]
          ? new Date(searchData.time[0]).getTime()
          : null;
      let finishTime =
        searchData && searchData.time[1]
      ...
      let params = {
        ...
        typeCode:
          searchData && searchData.typeCode ? searchData.typeCode : null,
        enable:
          searchData && searchData.status ? Number(searchData.status) : null,
        addTime: addTime,
          ...
          this.$api.api
            ...
            .then(res => {
              if (res.data.retcode === this.$config.RET_CODE.SUCCESS_CODE) {
                ...
                res.data.retcode === this.$config.RET_CODE.ERROR_CODE
              ) {
                this.$message({
                  type: "error",
                  message: res.data.retmsg
    ...
    submitAddedEditorForm() {
      ...
      if (this.handleType === "add") {
        ...
        //编辑
        let editPar = { ...params, id: this.userId };
        this.$api.api
          .updateAdNews(editPar)
          .then(res => {
            if (res.data.retcode === this.$config.RET_CODE.SUCCESS_CODE) {
              this.submitLoading = false;
              this.$message({
