<template>
  <div id="userMaeage">
    <el-row>
      <el-col :span="6">
        <el-input class="mar-r-10 mar-b-20" v-model="nameTxt" placeholder="用户名"></el-input>
      </el-col>
      <el-col :span="12">
        <el-button class="mar-l-10 mar-b-20" @click="queryUser">查询</el-button>
        <el-button class="mar-l-10 mar-b-20" type="primary" @click="addUser">新增</el-button>
        <el-button class="mar-l-10 mar-b-20" type="primary" @click="editUser">修改</el-button>
        <el-button class="mar-l-10 mar-b-20" type="warning" @click="showDialog">分配角色</el-button>
      </el-col>
    </el-row>

    <!-- 表格 -->
    <template>
      <el-table v-loading="loading" element-loading-text="拼命加载中" element-loading-spinner="el-icon-loading" element-loading-background="rgba(0, 0, 0, 0.8)" border :data="tableDatas" style="width:100%" @selection-change="handleSelectionChange">
        <el-table-column type="index" label=" "></el-table-column>
        <el-table-column type="selection"></el-table-column>
        <el-table-column prop="name" label="用户名"></el-table-column>
        <el-table-column prop="email" label="邮箱"></el-table-column>
        <el-table-column prop="phone" label="手机号"></el-table-column>
        <el-table-column prop="tel" label="固话"></el-table-column>
        <el-table-column prop="enable" label="状态" :formatter="stateFormat">
        </el-table-column>
        ...
        <el-table-column label="操作">
          <template slot-scope="scope">
            <el-button v-if="scope.row.enable == 1" size="mini" type="primary" :loading="btnloading" @click="handleEnabled(scope.$index, scope.row)">启用</el-button>
            <el-button v-else size="mini" type="danger" :loading="btnloading" @click="handleDisabled(scope.$index, scope.row)">禁用</el-button>
            <el-button size="mini" :loading="btnloading" @click="userDetail(scope.$index, scope.row)">详情</el-button>
          </template>
        </el-table-column>
      </el-table>
    </template>
    ...
    <!-- 弹框 -->
    <el-dialog title="角色分配" :visible.sync="allotRole" @close="closeDialog" center>
      <el-table :data="gridData" ref="dialogTable" border style="width:100%" @selection-change="dialogHandleSelectionChange" center>
        <el-table-column type="selection"></el-table-column>
        <el-table-column property="roleName" label="角色名称"></el-table-column>
        <el-table-column property="remark" label="备注说明"></el-table-column>
        <el-table-column property="id" v-if="show"></el-table-column>
      </el-table>
      <!-- 分页 -->
      <el-row class="pad-t-20 pad-b-20 bor-1">
        <div class="text-center">
          <el-pagination background @size-change="dialogSetAuctionSizeChange" @current-change="dialogSetAuctionCurrentChange" :current-page="dialogTotalPage" :total="dialogTotal" :page-size="dialogTotalPage" :page-sizes="[5,10, 20, 30, 40]" layout="total,sizes,prev,pager,next,jumper"></el-pagination>
        </div>
      </el-row>
