<template>
  <div id="menberList">
    <el-row>
      <el-col :span="6">
        <el-input class="mar-r-10 mar-b-20" v-model="nameTxt" placeholder="商户名"></el-input>
      </el-col>
      <el-col :span="12">
        <el-button class="mar-l-10 mar-b-20" @click="queryMember" icon="el-icon-select">查询</el-button>
        <el-button class="mar-l-10 mar-b-20" type="primary" @click="editMember" icon="el-icon-edit">修改</el-button>
      </el-col>
    </el-row>
    ...
    <template>
      <el-table v-loading="loading" element-loading-text="拼命加载中" element-loading-spinner="el-icon-loading" element-loading-background="rgba(0, 0, 0, 0.8)" border :data="tableDatas" style="width:100%" @selection-change="handleSelectionChange">
        ...
        <el-table-column prop="reviewState" label="审核状态" :formatter="reviewFormat"></el-table-column>
        <el-table-column prop="state" label="用户状态" :formatter="enableFormat"></el-table-column>
        ...
        <el-table-column label="操作">
          <template slot-scope="scope">
            <el-button v-if="scope.row.enable == 1" size="mini" type="primary" @click="handleEnabled(scope.$index, scope.row)">启用</el-button>
            <el-button v-else size="mini" type="danger" @click="handleDisabled(scope.$index, scope.row)">禁用</el-button>
            <el-button v-if="scope.row.reviewState == 2 && null != scope.row.reviewId" size="mini" type="primary" @click="memberAudit(scope.$index, scope.row)">审核</el-button>
            <el-button size="mini" @click="memberInfo(scope.$index, scope.row)">详情</el-button>
          </template>
        </el-table-column>
      </el-table>
    </template>
    ...
    <!-- 弹框 -->
    <el-dialog title="用户审核" :visible.sync="showAudit" @close="closeDialog" center>
      <el-row>
        <el-radio v-model="radio" label="0">通过</el-radio>
        <el-radio v-model="radio" label="1">不通过</el-radio>
      </el-row>
      <el-input class="mar-t-20" type="textarea" :rows="5" placeholder="请输入原因" maxlength="100" show-word-limit v-model="auditCause">
      </el-input>

      <el-button class="mar-t-20" size="mini" type="primary" :loading="btnloading" @click="submitAudit">提交</el-button>
    </el-dialog>
    <!-- 弹框结束 -->
  </div>
</template>
